<?
	...
	if ($action == "new") {
		$txtsql = "SELECT max(tblshop.shopid) as maxid FROM tblshop ";
		$txtrs = mysql_query($txtsql);
		$data = mysql_fetch_assoc($txtrs);
		$shopid =substr($data['maxid']+100001,1,5);
	
	}else if ($action == "edit") {
		$txtsql = "SELECT tblshop.* FROM tblshop WHERE tblshop.shopid ='".$shopid."' ";
		$txtrs = mysql_query($txtsql);
		$data = mysql_fetch_assoc($txtrs);
		$shopid = $data['shopid'];
	}
...
function deletedata(id){
	if(!confirm('คุณต้องการลบข้อมูลใช่หรือไม่'))
		return;
	window.location.href = '_shop.php?action=delete&shopid='+id;
}
...
function savedata(){
	...
	if(f.shopname.value==''){
		alert('ชื่อร้านอาหารเป็นช่องว่าง กรุณากรอกข้อมูลด้วยค่ะ');
		f.shopname.focus(); 
	}else
		f.submit();
}
                      ...
                      <tr>
                        ...
                        <table width="100%" border="0" cellpadding="1" cellspacing="2" class="tablecomment">
                          <tr>
                            ...
                            <a class="a1" href="_shop.php?action=new"><img src="png/blue-02.png" width="32" height="32" /></a><br />
                            <a class="a1" href="_shop.php?action=new">เพิ่มรายการ</a>
                            </td>
                            <td align="center" bgcolor="#F8F8F8" class="tb-line3" ><span class="text01">การจัดการข้อมูลร้านอาหาร</span></td>
                            <? } ?>
                           	<? if (!empty($action)) { ?>
                           <td align="center" bgcolor="#F8F8F8" class="tb-line3" ><span class="text01"><?=$action=="new" ? "การเพิ่มข้อมูลร้านอาหาร" : "การแก้ไขข้อมูลร้านอาหาร"?></span></td>
                           ...
                           <a class="a1" href="javascript:savedata()"><img src="png/green-36.png" width="32" height="32" /></a><br />
                            ...
                            <? } ?>
                            <td width="15%" align="center"><a class="a1" href="_shop.php"><img src="png/yellow-25.png" width="32" height="32" /></a><br />
                                          <a class="a1" href="_shop.php">ย้อนกลับ</a></td>
                          ...
                          <table width="100%" border="0" cellpadding="1" cellspacing="2" bgcolor="#F9F9F9" class="tabletopic2">
                            ...
                            <tr>
                              ...
                              <td width="79%" class="tbline_b_dash"><label>
                                <input name="shopid" type="text" value="<?=$shopid?>" readonly="readonly" class="readonly_" id="shopid" size="5" maxlength="5" />
                              </label></td>
                            </tr>
                            <tr>
                              <td width="21%" align="right" class="tbline_b_dash">ชื่อร้านอาหาร :</td>
                              <td width="79%" class="tbline_b_dash"><label>
                                <input name="shopname" type="text" value="<?=$data['shopname']?>" class="inputtext" id="shopname" size="70" maxlength="80" />
                              </label></td>
                            </tr>
                            <tr>
                      ...
                      <td>
                            <table width="100%" border="0" cellspacing="2" cellpadding="1">
                              <tr>
                                ...
                                <? if (!empty($data['picture01'])) { ?><a class="a1" href="picture/<?=$data['picture01']; ?>" class="lightbox"  rel="me"><img src="picture/<?=$data['picture01']?>" width="160" height="120" /></a><? } ?>                                </td>
                                <td width="34%" align="center" valign="top">
                                <? if (!empty($data['picture02'])) { ?>
    							<a class="a1" href="picture/<?=$data['picture02']; ?>" class="lightbox"  rel="me">
                              ...
                              <tr>
    							...
    							<a class="a1" href="picture/<?=$data['picture03']; ?>" class="lightbox"  rel="me">
                                <img src="picture/<?=$data['picture03']?>" width="160" height="120" />                                </a>
                                <? } ?>                                </td>
                                <td align="center" valign="top">
                                <? if (!empty($data['picture04'])) { ?>
    							<a class="a1" href="picture/<?=$data['picture04']; ?>" class="lightbox"  rel="me">
                      ...
                      <tr>
                        ...
                        <input name="statusfind" type="hidden" value="ok" />
                          <table width="100%" border="0" cellpadding="1" cellspacing="2" bgcolor="#0061FE"  class="tablecomment">
                            <tr>
                              <td width="32%" align="right" bgcolor="#0A9CFF">ค้นหาชื่อร้านอาหาร:</td>
                      ...
                      <tr>
                        ...
                        <table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#999999" class="tablecomment">
                          <tr>
                            ...
                            <td width="57%" align="center" bgcolor="#F9F9F9" class="txt01">ชื่อร้านอาหาร</td>
							...
							<?
                                $txtsql = "SELECT tblshop.* FROM tblshop  ";
                                if ($_POST['statusfind'] =="ok") {
                                    $txt = "";
                                    if (!empty($_POST['txtfind'])) {
                                        $txt = $txt."WHERE tblshop.shopname like '%".$_POST['txtfind']."%'";
                                    }
                                }
                                $txtsql = $txtsql.$txt." ORDER BY shopid ";
                                $txtrs = mysql_query($txtsql);
                                $i=1;
                            ...
                            ?>                                                            
                                  <tr>
                                    ...
                                    <td align="left" bgcolor="#FCFCFD" class="text01"><?=$row['shopname']?></td>
                                    <td align="center" bgcolor="#FCFCFD"><? if (!empty($row['picture01'])) { ?><a class="a1" href="picture/<?=$row['picture01']; ?>" class="lightbox"  rel="me"><img src="picture/<?=$row['picture01']?>" width="160" height="120" /></a><? } ?></td>
                                    <td align="center" bgcolor="#FCFCFD"><a class="a1" href="_shop.php?action=edit&shopid=<?=$row['shopid']?>"><img src="png/green-22.png" width="24" height="24" /></a> |<a class="a1" href="#" onClick="javascript:if(confirm('คุณต้องการลบข้อมูล ใช่หรือไม่?')==true){window.location='_shop.php?action=delete&shopid=<?=$row["shopid"];?>';}"> <img src="png/red-16.png" width="24" height="24" /></a></td>
                                  </tr>
                             <? } ?>
						  <? if ($i==1) { ?>
                          <tr>
...
<?
	if ($action=="delete") {
		$txtsql="DELETE FROM tblshop  WHERE shopid='".$shopid."'";		
		mysql_query($txtsql) or die(mysql_error());
		$msg = "ลบข้อมูลร้านอาหารเรียบร้อยแล้วครับ";
		echo"<script language=javascript>";
		echo"alert('$msg')"; 
		echo"</script>";
		echo "<meta http-equiv='refresh' content='0;URL=_shop.php'>";		
		exit();																		
	}
	if ($_POST['save']=="yes") {
		//$name = $row['custname']." ".$row['custsname'];
		if ($action=="new") {		
			foreach($_FILES[picfile][name] AS $i => $filepic) {   
				if ($filepic != '') { //   
					...
					$picfile[$i]="p".$_POST['shopid']."_".$i.".".$extension[1];
			...
			$txtsql = "insert into tblshop (shopid,shopname,detail,location,travel,picture01,picture02,picture03,picture04) values('".$_POST['shopid']."','".$_POST['shopname']."','".$_POST['detail']."','".$_POST['location']."','".$_POST['travel']."','".$picfile[0]."','".$picfile[1]."','".$picfile[2]."','".$picfile[3]."' ) ";
			mysql_query($txtsql) or die(mysql_error());
		} else {
			$txtsql = "update tblshop set shopname ='".$_POST['shopname']."',detail ='".$_POST['detail']."',location ='".$_POST['location']."',travel ='".$_POST['travel']."' ";
			foreach($_FILES[picfile][name] AS $i => $filepic) {  
				if ($filepic != '') { // ตรวจสอบว่ามีการเลือกไฟล์มาหรือไม่  
					$extension = explode(".",$filepic);
					$picfile[$i]="sp".$_POST['shopid']."_".$i.".".$extension[1];
					copy($_FILES[picfile][tmp_name][$i],"picture/".$picfile[$i]);
					$cnt = substr($i+101,1,2);
					$txtsql = $txtsql.",picture".$cnt."='".$picfile[$i]."'";
				} 
			}			
			$txtsql = $txtsql." WHERE shopid ='".$shopid."' ";
			
			mysql_query($txtsql) or die(mysql_error());
		}
		$msg = "บันทึกข้อมูลร้านอาหารเรียบร้อยแล้วครับ";
		echo"<script language=javascript>";
		echo"alert('$msg')"; 
		echo"</script>";
		echo "<meta http-equiv='refresh' content='0;URL=_shop.php'>";		
