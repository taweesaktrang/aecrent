                      ...
                      <tr>
                        ...
                        <input name="statusfind" type="hidden" value="ok" />
                          <table width="100%" border="0" cellpadding="1" cellspacing="2" bgcolor="#F9F9F9"  class="tablecomment">
                            <tr>
                              <td width="29%" height="41" align="right" bgcolor="#F9F9F9" class="tbline_b_dash">โปรดระบุเดือน / ปีที่ :</td>
                              <td width="71%" bgcolor="#F9F9F9" class="tbline_b_dash">
                              <label>
                              	<?
									$mm=date("m");
								?>
                              	<select name="mm" class="inputtext" id="mm">
                                <option value="01" <?=$mm=='1' ? 'selected' : ''?>>มกราคม</option>
                                <option value="02" <?=$mm=='2' ? 'selected' : ''?>>กุมภาพันธ์</option>
                                <option value="03" <?=$mm=='3' ? 'selected' : ''?>>มีนาคม</option>
                                <option value="04" <?=$mm=='4' ? 'selected' : ''?>>เมษายน</option>
                                <option value="05" <?=$mm=='5' ? 'selected' : ''?>>พฤษภาคม</option>
                                <option value="06" <?=$mm=='6' ? 'selected' : ''?>>กรกฎาคม</option>
                                <option value="07" <?=$mm=='7' ? 'selected' : ''?>>มิถุนายน</option>
                                <option value="08" <?=$mm=='8' ? 'selected' : ''?>>สิงหาคม</option>
                                <option value="09" <?=$mm=='9' ? 'selected' : ''?>>กันยายน</option>
                                <option value="10" <?=$mm=='10' ? 'selected' : ''?>>ตุลาคม</option>
                                <option value="11" <?=$mm=='11' ? 'selected' : ''?>>พฤศษจิกายน</option>
                                <option value="12" <?=$mm=='12' ? 'selected' : ''?>>ธันวาคม</option>                            
                                </select>
                                &nbsp;ปี
                                :
                                <select name="yyyy" class="inputtext" id="yyyy">
                                    <?
                                    $yy = date("Y")+543;
                                    for($yyyy=$yy-1;$yyyy<=$yy+5;$yyyy++){ ?>
                                <option <?=$yyyy==$yy ? 'selected' : ''?>><?=$yyyy;?></option>
